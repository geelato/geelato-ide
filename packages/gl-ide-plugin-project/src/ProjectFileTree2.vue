...
//                    alert('根节点不能删除！')
      ...
      savePage: function () {
        let that = this
        console.log('that.editingPage>save>', that.editingPage)
//        editingPage.component = editingPage.component.outerHTML || editingPage.component
        that.api.save(that.$gl.entityNames['platform-ide'].dev.pageConfig, {
          id: that.editingPage.id,
          extendId: that.editingPage.extendId,
          type: that.editingPage.type,
          code: that.editingPage.code,
          description: that.editingPage.description,
          content: that.editingPage.content
        }).then(function (res) {
          that.editingPage.id = res.data
//          that.$gl.ui.showMsg('页面保存成功', 'success', '')
        }).catch(function (e) {
//          that.$gl.ui.showMsg('页面保存失败', 'fail', '')
        })
      },
      /**
       * 新节点，则从模板中加载页面
      ...
      openPage(event, item) {
        let that = this
        that.api.query(that.$gl.entityNames['platform-ide'].dev.pageConfig, {extendId: item.node.id}, 'id,type,code,description,content').then(function (res) {
//          that.editingPage.reset(res.data[0])
          console.log('res.data[0]>', res.data[0])
          // that.editorStore.reset(new SimplePageDefinition(res.data[0], true))
          // console.log('editingPage>', that.editingPage)
        }).catch(function (e) {
          console.error(e)
          that.$gl.ui.showMsg('从服务端获取、解析信息失败！')
        })
      },
      /**
       *
      ...
      removePage(nodeId) {
        let that = this
        // 两张表的删除，合在一个事务中
        that.api.delete(that.$gl.entityNames['platform-ide'].dev.pageConfig, {extendId: nodeId})
        that.api.delete(that.$gl.entityNames['platform-core'].common.treeNode, {id: nodeId})
      },
      loadCachePage: function (extendId) {
        let that = this
        console.log('loadCachePage extendId>' + extendId + '>', this.loadedPages[extendId])
        that.editingPage = this.loadedPages[extendId]
        that.editingPage.extendId = extendId
      },
      updateNode(node, status, cancelled) {
        ...
        if (node.parent === '#') {
          ...
          that.api.save(that.$gl.entityNames['platform-core'].common.treeNode, treeNode).then(function (res) {
